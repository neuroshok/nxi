...
void nxi_core_error(nxi::core* core, const char* message) { core->user_system().focus().error(message); }
...
namespace nxi::api
{
    ...
    void core::log(const QString& message) { core_.error(message); };

    void core::error(const std::string& message) { core_.error(QString::fromStdString(message)); };
