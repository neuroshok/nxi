...
namespace ui
{
    user_system::user_system(ui::core& ui_core)
        : ui_core_{ ui_core }
    {
        connect(&ui_core_.nxi_core().user_system(), &nxi::user_system::event_add, [this](nxi::user_session& session)
        {
            sessions_.emplace_back(std::make_unique<ui::user_session>(ui_core_, session) );
        });

        connect(&ui_core_.nxi_core().user_system(), &nxi::user_system::event_unload, [this](nxi::user_session& session)
        {
            nxi_trace_event("");
